...
function validate_project_config(config: PackageReeConfig, project_root: string): ReeProjectConfig | null {
	...
	const translation_root = validate_relative_path(config.translation_root, project_root);
	if (!template_roots || !component_roots || !translation_root) return null;
